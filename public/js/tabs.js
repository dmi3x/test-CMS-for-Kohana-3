var Tabs = function (addBoxId, editBoxId, pageNum, ajaxUrl) {
...
Tabs.prototype.init = function(addBoxId, editBoxId, pageNum, ajaxUrl) {
    var self = this;
    if(!addBoxId) {
...
Tabs.prototype.showAddBox = function() {
    var self = this;
    $(self.config.addBoxId).dialog('open');
}

Tabs.prototype.showEditBox = function(id) {
    var self = this;
    ...
    $.ajax({type: "POST",
	  ...
	  success: function(res){
	      ...
	      if(res.substr(0, 1)==1) {
		    ...
		    $('form input[name=link]', $(self.config.editBoxId)).val(arr.link);
